...
for (n, idx) in enumerate(matching_idx)
        ...
        rainfall_matrix[n,3] = max(rainfall[idx],rainfall[idx-1],rainfall[idx-2])
        rainfall_matrix[n,4] = rainfall[idx]+rainfall[idx-1]+rainfall[idx-2]
end

colnames = ["day", "1 day", "3 days max rainfall", "3 days total rainfall"]
df = DataFrame(rainfall_matrix, Symbol.(colnames))
CSV.write("../data/observations/rainfall/3.csv", df)
