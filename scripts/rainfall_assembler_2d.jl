...
for (n, idx) in enumerate(matching_idx)
        ...
        flow_matrix[n,3] = max(flow[idx],flow[idx-1])
        flow_matrix[n,4] = flow[idx]+flow[idx-1]
end

colnames = ["day", "1 day flow", "2 days max flow", "2 days total flow"]
df = DataFrame(flow_matrix, Symbol.(colnames))
CSV.write("../data/observations/flow_max/2.csv", df)
