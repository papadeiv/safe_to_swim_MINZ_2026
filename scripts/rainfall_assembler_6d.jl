...
for (n, idx) in enumerate(matching_idx)
        ...
        rainfall_matrix[n,3] = max(rainfall[idx],rainfall[idx-1],rainfall[idx-2],rainfall[idx-3],rainfall[idx-4],rainfall[idx-5])
        rainfall_matrix[n,4] = rainfall[idx]+rainfall[idx-1]+rainfall[idx-2]+rainfall[idx-3]+rainfall[idx-4]+rainfall[idx-5]
end

colnames = ["day", "1 day", "6 days max", "6 days total"]
df = DataFrame(rainfall_matrix, Symbol.(colnames))
CSV.write("../data/observations/rainfall/6.csv", df)
